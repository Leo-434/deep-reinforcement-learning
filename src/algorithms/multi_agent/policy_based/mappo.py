...
class MAPPO(BaseAlgorithm):
    ...
    Actor takes local obs. Critic takes global state (or concatenated obs) to reduce variance.
    """
    def __init__(self, cfg: dict, device: torch.device):
        ...
        # Centralized Critic (Input: Global State)
        self.critic = build_mlp(self.state_dim, 1, hidden_dims).to(self.device)
    ...
    def update(self, batch: dict, steps: int) -> dict:
        ...
        states = batch["states"]        # [Seq, StateDim]
        acts = batch["acts"].long()     # [Seq, N_Agents, 1]
        rews = batch["rews"][:, 0, :]   # [Seq, 1], assumed shared reward in MAPPO
        next_obs = batch["next_obs"]    # [Seq, N_Agents, ObsDim]
        next_states = batch["next_states"]# [Seq, StateDim]
        dones = batch["done"][:, 0, :]  # [Seq, 1]
        avail_actions = batch.get("avail_actions", None)
        
        seq_len = obs.shape[0]
        
        # We need values from global states
        with torch.no_grad():
            old_vals = self.critic(states) # [Seq, 1]
            next_vals = self.critic(next_states) # [Seq, 1]
            ...
            # Tile advantages and returns for each agent to match actor shapes
            # advs: [Seq, 1] -> [Seq, N_Agents, 1]
            advs_agent = advs.unsqueeze(1).repeat(1, self.num_agents, 1)
            rets_agent = rets.unsqueeze(1).repeat(1, self.num_agents, 1)
            
            # Flat tensors for actors
            obs_flat = obs.view(seq_len * self.num_agents, self.obs_dim)
            acts_flat = acts.view(seq_len * self.num_agents, 1)
            advs_flat = advs_agent.view(seq_len * self.num_agents, 1)
        ...
        for _ in range(self.ppo_epochs):
            ...
            # Critic loss (evaluated on single global state trajectory)
            curr_vals = self.critic(states)
            critic_loss = F.mse_loss(curr_vals, rets)
